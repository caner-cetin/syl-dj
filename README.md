...
``album artist ID``,``album ID``,``artist ID``,``recording ID``,``work ID``. All types of ID's are saved into database
when high level data is uploaded and can be queried by respective endpoints.
